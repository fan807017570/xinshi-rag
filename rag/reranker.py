...
class BGEReranker:
    ...
    def rerank(self, query, docs, top_k):
        if not docs or top_k <= 0:
            return []
        if len(docs) > self.candidate_limit:
            raise RuntimeError(
                "reranker candidate_limit is smaller than the retrieved "
                f"candidate count: {self.candidate_limit} < {len(docs)}"
            )
        if len(docs) == 1:
            return self._annotate_ranked_documents([(docs[0], None)])
        if top_k >= len(docs):
            return self._annotate_ranked_documents([
        ...
        if top_k >= len(docs_for_rerank):
            return self._annotate_ranked_documents([
                (doc, None) for doc in docs[:top_k]
            ])
        ...
        return self._annotate_ranked_documents(ranked[:top_k])

    @staticmethod
    def _annotate_ranked_documents(ranked_documents):
        """Attach rerank position and score metadata to returned documents."""
        documents = []
