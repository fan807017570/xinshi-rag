...
启动方式（任选其一，均需已配置 Milvus / 模型）：
    ...
    python -m uvicorn rag.server:app --host 0.0.0.0 --port 8765
...
async def health():
    try:
        return {"status": "ok", **get_rag_runtime_status()}
...
if _HAS_PYTHON_MULTIPART:
    ...
    async def upload_doc(
        ...
        if reindex:
            try:
                from rag.ingest import ingest as _ingest
                from rag.application import reload_vectorstore

                loop = asyncio.get_running_loop()
                coverage = await loop.run_in_executor(None, _ingest)
...
@app.get("/api/docs/index", summary="查询 Milvus 索引来源覆盖率")
async def index_coverage():
    from rag.index_coverage import inspect_index_coverage
    from rag.ingest import list_ingest_source_paths

    loop = asyncio.get_running_loop()
    report = await loop.run_in_executor(
        None,
        inspect_index_coverage,
        list_ingest_source_paths(),
    )
