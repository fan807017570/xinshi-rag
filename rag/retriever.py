...
from rag.milvus_vectorstore import create_milvus_vectorstore
...
def get_vectorstore():
    """Build the explicitly configured retrieval backend.

    Milvus connection or dependency failures are intentionally propagated. The
    service must not silently switch retrieval algorithms after startup.
    """
    if USE_MILVUS:
        vectorstore = _build_milvus_vectorstore()
        log.info(
            "retriever backend=milvus collection=%s endpoint=%s:%s",
            COLLECTION_NAME,
            MILVUS_HOST,
            MILVUS_PORT,
        )
    ...
    log.warning("retriever backend=local selected explicitly; Milvus is disabled")
    return LocalVectorStore(_load_documents())


def get_vectorstore_backend(vectorstore: Any) -> str:
    """Return a stable backend name for logs, health checks, and metrics."""
